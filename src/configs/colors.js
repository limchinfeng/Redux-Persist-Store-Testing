export const white = '#ffffff';
export const gray1 = '#f2f2f2'; // gray5
export const gray2 = '#E9E9E9'; // gray4
export const gray3 = '#BDBDBD'; // gray2
export const gray4 = '#80879A'; // second
export const gray5 = '#1D2338'; // black
export const gray6 = '#F6F8FB'; // gray6
export const gray7 = '#3A3B3C'; // gray4 dark
export const gray8 = '#242526'; // gray2 dark
export const gray9 = '#B0B3B8'; // gray2 dark
export const gray10 = '#18191A'; // gray2 dark
export const gray11 = '#E1E3E8'; // gray5 dark
export const black = '#000000';
export const blue = '#0B69FF';
export const cyan = '#56CCF2';
export const aqua = '#00B8D4';
export const green = '#2BBD69';
export const orange = '#F2994A'; // Orange
export const darkOrange = '#FF5200'; // orange
export const yellow = '#FDC309';
export const red = '#DA291C';
export const navy = '#005082';
export const darkBlue = '#063D80';
export const lightGray = '#ECECEC';
export const lightBlue = '#4A90E2';
export const darkRed = '#8D0011';
export const darkGrey = '#8c8c8c';
export const darkYellow = '#FBCD02';
export const inputBackground = '#F8FAFC';
export const inputPlaceholder = '#B0CDEA';
export const inputBorder = '#B0CDEA';
export const success = '#13BECE';

